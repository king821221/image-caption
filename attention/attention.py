...
class BahdanauAttention(tf.keras.Model):
  ...
  def __init__(self,
      ...
      self.W1 = tf.keras.layers.Dense(units, name='{}/W1'.format(self.name))
      self.W2 = tf.keras.layers.Dense(units, name='{}/W2'.format(self.name))
      self.V = tf.keras.layers.Dense(1, name='{}/V'.format(self.name))
      self.D = tf.keras.layers.Dropout(attention_dropout)
      self.activation = activation

  def call(self, features, hidden, training=False, bias=None):
      # features: [batch_size, feature_length, feature_dim]
      # hidden : [batch_size, hidden_size]

      # hidden_with_time_axis: [batch_size, 1, hidden_size]
      hidden_with_time_axis = tf.expand_dims(hidden, 1)

      attention_hidden_layer = self.activation(
      ...
      # attention_weights: [batch_size, feature_length, 1]
      attention_weights = self.D(attention_weights, training=training)
...
# Transformer based attention is NOT tried yet
...
class MultiHeadAttention(tf.keras.Model):
  ...
  >>> output_tensor, weights = model(target, source,
  ...
  def _build_attention(self, rank):
    ...
    axis = -1
    self._softmax = advanced_activations.Softmax(axis=axis)
  ...
  def _compute_attention(self,
    ...
    # attention scores.
    print("key {} query {}".format(key.shape, query.shape))
    attention_scores = tf.einsum(self._dot_product_equation, key,
                                               query)

    #print("query {}  key {} attention {}".format(query.shape, key.shape, attention_scores.shape))

    attention_scores = self._masked_softmax(attention_scores, attention_mask)
    #print("attention masked {}".format(attention_scores.shape))
    ...
    #print("attention scores {}".format(attention_scores_dropout.shape))
    #print("value scores {}".format(value.shape))
  ...
  def call(self,
    ...
    print("atten query {} key {} value {}".format(
    ...
    print("atten_proj query {} key {} value {}".format(
      query.shape, key.shape, value.shape
    ))
